...
app.get('/scrapeUnits', function(req, res){
	...
	unit_list_url = 'http://bravefrontierglobal.wikia.com/wiki/Unit_List?action=render';


	// Load the main unit list and gather up an array of urls for each unit.
	var unit_group_list = [];
	request(unit_list_url, function(error, response, html){
		//console.log(response);
		if(!error){
			var $ = cheerio.load(html);
			$('#flytabs_unitlist').find('li').each(function(){
	...
	function fetchUnitsFromGroup (value) {
	...
	request(value, function(error, response, html){
		...
		if(!error){
			var $ = cheerio.load(html);
							...
							var series = $(statsTable).find("th:contains('Chain')").siblings().find('a').eq(0).attr('title');
							json.series = decodeURI(series);
					...
					fs.writeFile('units/' + json.name.replace(/ /g,'') + '.json', JSON.stringify(json, null, 4), function(err){
	...
	url = 'http://bravefrontierglobal.wikia.com/wiki/Unit_List?action=render';
...
app.get('/scrapeOneUnit', function(req, res){
	request('http://bravefrontierglobal.wikia.com/wiki/Dictator_Lilly_Matah', function(error, response, html){
		//console.log(response);
		if(!error){
			var $ = cheerio.load(html);
							...
							// BB Fill
							var bb_fill = parseInt($('#unit-info-0').find("th:contains('Normal')").parent().next().children().first().next().next().text().trim().replace(/[^\d\.]/gi, ""), 10);
							// @TODO We need to split up 4 digits into XX ~ XX
							// if (bb_fill.length > 2) {
